...
| **MiniMaxH3CloudDirector** | ★ 云端多模态导演：节点正面只暴露云端连接预设、媒体上传策略和生成预算，供应商 host、默认模型、thinking/wire/schema 由受控预设与 adapter 解析；当前包含 DeepSeek 与 Gemini 原生 G0 预设。Key 从环境变量或 ComfyUI 用户目录本地凭据文件读取，不进入工作流；复用相同媒体序列、模块、Prompt IR、编译校验和失败阻断 |
...
`<Picture N>` 编号。帧数上限为 300；DeepSeek 官方端点会把序列 Data URL 控制在约 28 MiB，
...
全部已选帧，不会静默丢帧。当前不承诺 600 帧内联请求，超过 300 帧应先抽样；后续应评估文件上传
或真正的时间线分块接口，而不是继续堆 base64。
...
- `api_reasoning=auto` 且模型 ID 能识别为 Qwen3.8 或 Gemma4 时：素材分析调用允许思考，最终固定 JSON
  编译调用关闭思考。连接本机 LM Studio 时，最终调用会优先走原生 `/api/v1/chat` 的
  `reasoning=off`；本机 Q6 实测为 `0 reasoning token`。显式选择 `on` 或 `off` 仍会覆盖自动策略。
...
- 点击“刷新 / 选择云端模型”才会按当前连接预设向官方 ListModels 发出一次请求；不会在打开工作流时
  自动联网。Key 只由 ComfyUI 后端从当前 provider 的本地凭据/环境变量解析，浏览器只收到模型 ID、
  显示名、生成方法与公开 token 上限。选择器支持搜索、恢复预设默认值，并用 `★` 标出适合文本
  Prompt IR 的候选；“账户可见”不等于已经通过本插件的多图、thinking 与严格 JSON 实测。
- “参考图与视频帧的上传方式”直接说明媒体处理：`auto/single` 将按连接顺序编号的参考图与所选
  `<Video 1>` 连续帧同次上传；`staged` 先分别分析再汇总。节点底部的“参考图与视频帧上传说明”
...
```
  ...
  DeepSeek 与 Gemini 分别使用 `deepseek_default` / `gemini_default`，可共存于同一凭据文件且不会
...
- Gemini 使用 `x-goog-api-key`、原生 `systemInstruction + contents`、图片 `inlineData` 与
...
- DeepSeek 的供应商输出能力与 Prompt IR 项目预算分开计算。旧通用导演默认 8192，云端导演
  默认 16384；旧工作流中 `131328` 这类超过控件上限的值会在运行时规范化为 16384，并写入报告，
  而不是照单全收。
...
当前功能分支工作树：`151 passed`（WF Python，2026-08-27；1 条环境自带 `pynvml`
弃用警告不属于测试失败）。
...
- **云端模型列表接口**（`/minimaxh3lab/cloud/models`）：只接受固定 provider/credential ID，出站 host
  由 adapter 固定；响应不含 Key 或供应商原始响应正文。DeepSeek 返回账户可见 ID，Gemini 只保留明确
  支持 `generateContent` 的条目，并单独标记而不冒充“已验证”。
